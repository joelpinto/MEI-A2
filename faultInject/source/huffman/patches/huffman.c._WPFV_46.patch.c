...
static void WriteFiles(HuffmanArchive* HA, int argc, FILE** f, char** codes, FILE* farchive)
{
...
static void WriteArchive(HuffmanArchive* HA, int argc, char** argv, FILE** f, char** codes)
{
    int i;
...
static unsigned char DecodeByte(HuffmanNode* nodes, int root, unsigned char* bytes, int* bit)
{
    int element, shift;
    unsigned char mask;
...
static void DecodeAndWriteFiles(HuffmanArchive HA, char* folder_out, FILE* ar)
{
    ...
    for (i = 0;i < HA.ah.file_entries_n;i++){
        ...
        for (j = 0;j < HA.fh[i].file_size;j++){
            write_bytes[j] = DecodeByte(HA.nodes, nodes_n, read_bytes, &bit);
